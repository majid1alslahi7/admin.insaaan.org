...
export default function CreateUserPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      toast.success('تم إضافة المستخدم بنجاح', { icon: '✓', style: { background: '#159C4B', color: '#fff' } });
      router.push('/users');
    } catch {
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">الحقول مطابقة لقواعد إنشاء المستخدم في الباكند</p>
        </div>
      </div>
        ...
        <div className="card space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                value={form.status}
                onChange={(event) => setForm({ ...form, status: event.target.value as UserForm['status'] })}
                className="w-full p-3 border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-900"
              >
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={loading} className="btn btn-primary flex-1">
              <FaFloppyDisk /> {loading ? 'جاري الحفظ...' : 'حفظ المستخدم'}
            </button>
