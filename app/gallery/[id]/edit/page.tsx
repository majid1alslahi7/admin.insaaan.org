...
export default function EditGalleryPage() {
  ...
  useEffect(() => {
    ...
    if (id) galleryAPI.getOne(Number(id)).then(setForm).catch(() => router.push('/gallery'));
  }, [id, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await galleryAPI.update(Number(id), { ...form, program_id: Number(form.program_id) || null, project_id: Number(form.project_id) || null });
      toast.success('تم تحديث الصورة بنجاح', { icon: '🖼️', style: { background: '#EC4899', color: '#fff' } });
      router.push('/gallery');
    } catch {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-6"><Link href="/gallery" className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg"><FaArrowLeft /></Link><h1 className="text-2xl font-bold">تعديل صورة</h1></div>
      <motion.form initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit} className="space-y-6">
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="card space-y-4">
            ...
            <div><label>الترتيب</label><input type="number" value={form.order || 0} onChange={e => setForm({...form, order: +e.target.value})} className="w-full p-3 border rounded-xl" /></div>
        ...
        <div className="flex gap-4"><button type="submit" disabled={loading} className="btn btn-primary"><FaFloppyDisk /> {loading ? 'جاري الحفظ...' : 'تحديث الصورة'}</button><Link href="/gallery" className="btn bg-gray-200 dark:bg-gray-700">إلغاء</Link></div>
      </motion.form>
    </div>
  );
}
